...
| Trigger                                  | Handling                                                                                                                                                                                                      | Layer                 |
| ---------------------------------------- | ------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------------- | --------------------- |
| LLM timeout (>15s)                       | Throw `ExtractionError('timeout')`; UI shows fallback and retry.                                                                                                                                              | Service + UI.         |
| Rate limit (429)                         | Backoff once with jitter; if still failing, surface "Service is busy, please retry."                                                                                                                          | Service.              |
| API key missing or invalid               | Boot-time `env.ts` validation rejects the start when no AI provider key is set; UI shows the structured failure banner if a runtime call still 401s.                                                          | Boot check + UI flag. |
| 401 from active provider                 | Same UX as missing key for the user; engineer sees error in logs.                                                                                                                                             | Service.              |
